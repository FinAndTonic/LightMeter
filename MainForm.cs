...
namespace UATControl
{
	...
	MainForm : Form
	{
        ...
        private void DACBar_ValueChanged(object sender, EventArgs e)
        {
            ...
            //3.26*(722/1023) - .7 = 1.6
            double newValue = (3.3*DACBar.Value/1023);
        ...
        private void SendLPC_Click(object sender, EventArgs e)
        {
            ...
            ///The below all assumed the ASIP decoding, which I have not been using recently.
            //txBuf[0] = 0xC2;
            //txBuf[1] = 0x52;
            //txBuf[2] = 0xFF;
            ...
            txBuf[0] = (byte)(3);               //the number of bytes we're sending, that aren't this byte
            txBuf[1] = (byte)0x6C;             //"address" of the LPC

            txBuf[2] = (byte)((DACBar.Value & 0x0F00) >> 8);    //MSb
            txBuf[3] = (byte)(DACBar.Value & 0x0FF);          //LSB
        ...
        {
            Decimal num;
            Decimal target = Convert.ToDecimal(DAC_target.Text);
            Decimal vdd = Convert.ToDecimal(DAC_vdd.Text);

            num = ((target / vdd) * 4096);
            DAC_value.Text = Convert.ToString((Int16)num, 16);
        }

        private void DAC_vdd_TextChanged(object sender, EventArgs e)
        {
            UpdateDACData();
        }

        private void DAC_target_TextChanged(object sender, EventArgs e)
        {
            UpdateDACData();
        }

        private void upoll_start_Click(object sender, EventArgs e)
        {
            //if (!comport.IsOpen)
            //{
            //    Output.AppendText("You have not yet opened a COM port!\r\n");
            ...
            //comport.ReadExisting();     //this is here to start the real reading with a clean slate

            upoll_start.Visible = false;
            upoll_stop.Visible = true;

            Output.AppendText("Incomming data:\r\n");
        ...
        {
            //much thanks to: http://stackoverflow.com/questions/5468039/cross-thread-operation-not-valid-while-reading-data-on-serial-port
            //regarding Invoke

            //Output.AppendText(comport.ReadExisting());
        ...
        private void upoll_stop_Click(object sender, EventArgs e)
        {
            upoll_stop.Visible = false;
            upoll_start.Visible = true;

            Output.AppendText("Polling ended.\r\n");
        ...
        {
            byte[] rcvd = new byte[8];
            comport.Read(rcvd, 0, 8);

            string received = string.Format("{0:X2} {1:X2} {2:X2} {3:X2} {4:X2} {5:X2} {6:X2} {7:X2}\r\n",rcvd[0], rcvd[1], rcvd[2], rcvd[3], rcvd[4], rcvd[5], rcvd[6], rcvd[7]);
            Output.AppendText(received);
        }

        private void button_xdata_Click(object sender, EventArgs e)
        {
            byte[] txBuf = new byte[8];
            byte[] rxBuf = new byte[8];

            txBuf[0] = (byte)(2);               //the number of bytes we're sending, that aren't this byte
            txBuf[1] = (byte)0x6F;             //"address" of the LPC
            txBuf[2] = (byte)0xD0;              //pulled out of nothing: D to be "FPGA", "6" to be asking for xData, "7" to be asking for yData

            comport.Write(txBuf, 0, 3);

            //System.Threading.Thread.Sleep(250);

            comport.Read(rxBuf, 0, 1);

            //text_xdata.Text = Convert.ToString(rxBuf[0], 16);
            text_xdata.Text = string.Format("{0:X2}, {1}", rxBuf[0], rxBuf[0] - 127);
        }

        private void button_ydata_Click(object sender, EventArgs e)
        {
            byte[] txBuf = new byte[8];
            byte[] rxBuf = new byte[8];
            ...
            //text_ydata.Text = Convert.ToString(rxBuf[0], 16);
            text_ydata.Text = string.Format("{0:X2}, {1}", rxBuf[0],rxBuf[0]-127);
        }

        private void button_zo_Click(object sender, EventArgs e)
        {
            byte[] txBuf = new byte[8];
            byte[] rxBuf = new byte[8];
            ...
            txBuf[0] = (byte)2;
            txBuf[1] = (byte)0x6F;
            txBuf[2] = (byte)0xD2;      //D is still filler, only the last 2 bits matter: 00 01 10 = x, y, z
            ...
            text_zo.Text = string.Format("{0:X2}", rxBuf[0]);
        }
